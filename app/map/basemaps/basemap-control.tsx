...
export function BasemapControl({
		...
		buttonSizePx = 64, // Etwas grösser für bessere Haptik
		opacity,
		onOpacityChange,
	}: BasemapControlProps) {
					...
					<PopoverContent
						...
						<div className="flex flex-col">
							...
							<div className="bg-slate-50/50 p-4 pb-3 dark:bg-slate-900/50">
								...
								<h3 className="text-base font-semibold tracking-tight text-foreground">{selected.label}</h3>
							</div>
							...
							<div className="p-4 pt-2">
								...
								{showDescription && selected.description && (
									<div className="mt-4 rounded-xl bg-slate-100/50 p-3 dark:bg-slate-800/50">
										<p className="text-[11px] leading-relaxed text-muted-foreground/90">{selected.description}</p>
									</div>
								)}
							</div>
						</div>
