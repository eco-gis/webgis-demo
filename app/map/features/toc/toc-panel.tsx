...
export function TocPanel() {
	...
	const allItems = useMemo(() => [...(MAP_CONFIG.tocItems as TocItemConfig[]), ...dynamicItems], [dynamicItems]);

	const orderedItems = useMemo(() => {
		if (!order.length) return allItems;
		const itemMap = new Map(allItems.map((it) => [it.id, it]));
		const sorted = order.map((id) => itemMap.get(id)).filter(Boolean) as TocItemConfig[];
	...
	function handleDragEnd(event: DragEndEvent) {
		...
		if (over && active.id !== over.id) {
			...
			const newOrder = arrayMove(orderedItems, oldIndex, newIndex).map((it) => it.id);
			setOrder(newOrder);
		}
	}

	if (orderedItems.length === 0) {
		return <div className="p-8 text-center text-sm text-muted-foreground w-full">Keine Ebenen vorhanden</div>;
	}
					...
					<div className="flex flex-col gap-2 w-full min-w-0">
						{orderedItems.map((item) => (
							<SortableTocItem
								...
								labelsOn={labelsVisible[item.id] ?? item.defaultLabelsVisible ?? false}
